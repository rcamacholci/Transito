...
<body>
<%
    if(request.getParameter("NombreBanc")!=null){
    String nombre = request.getParameter("NombreBanc");
    nombre = nombre.toUpperCase();
    try{
        if(nombre.length()>0){
        modelo.adicionarBancos(nombre);
        modelo.getCon().commit();
        %>
        <script>
            alert("El Banco ha sido adicionado");
            window.parent.close();
        </script>
        <%
        }else{
        %>
            <script>
            alert("Error al adicionar Banco");
            window.parent.close();
