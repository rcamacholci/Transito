...
<html>
            ...
            window.onload = function () {
                if (!window.location.hash) {
                    ...
                    if (trs.length == 2) {
                        ...
                        /**/
                        var valor = document.getElementById('fk_vehiculo').value;
                        var ajax = new nuevoAjax();
                        if (window.parent.document.getElementById('aplicar') != null)
                            window.parent.document.getElementById('aplicar').disabled = true;
                        ajax.open("POST", "SetDescuento.jsp", true);
                        ajax.onreadystatechange = function () {
                            if (ajax.readyState == 4) {
                                if (ajax.status == 200) {
                                    window.parent.frames[4].location.href = "verLiquidacion.jsp?id=&tipo=";
                                } else {
                        ...
                        ajax.send("valor=" + valor + "&op=" + 1);
                        /**/
                    }
                }
            };

            function adicionarVEH(id, placa, ref) {
                if (window.parent.document.getElementById('aplicar') != null)
                    window.parent.document.getElementById('aplicar').disabled = true;
                var tabla = document.getElementById('tabla');
                var trs = tabla.getElementsByTagName('tr');
                ...
                for (var i = 0; i < trs.length; i++) {
                    if (trs[i].id == id) {
                        if (i == 1) {
                            ...
                            if (i == trs.length - 2 && referent == '2018') {
    ...
    <input type="hidden" name="fk_vehiculo" id="fk_vehiculo" value="<%=vehiculo.getId_vehiculo()%>" />
    <table width="100%" border="1" id="tabla" cellpadding="1" cellspacing="0">
        ...
        <tr id="<%=cartera.getId_cartera()%>" onMouseOver="sombrear(this)" onMouseOut="renovar(this)">
            <td align="center" style="cursor:pointer;">
                <img src="../html/images/adicionar.png" width="19" height="19" onClick="adicionarVEH(<%=cartera.getId_cartera()%>, '<%=placa%>', '<%=cartera.getReferencia()%>')"/>							</td>
