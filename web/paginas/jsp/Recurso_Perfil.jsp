...
response.setHeader ("Content-Disposition", "attachment;filename=\"RecursosPorPerfil.xls\"");
%> <tr>
<td>Generado por:</td>
...
<% long id_recurso = Long.parseLong(request.getParameter("recurso"));
java.util.List<Recurso> nombre = null;
nombre = model.consultarRecursoNombre(id_recurso);
Recurso nombre2 = nombre.get(0);
%>
...
<%
		...
		<th width="5%" scope="col"><span class="Estilo2">N°</span></th>
                <th width="10%" scope="col"><span class="Estilo2">NOMBRE DEL PERFIL</span></th>

              </tr>
              <%
     try{
         Sede sede = (Sede)session.getAttribute("sede");
         java.util.List<Recursos_Perfil> recurso_perfil = null;

          recurso_perfil = model.consultarRecurso_Perfil(Long.parseLong(request.getParameter("recurso")));
         if(recurso_perfil!=null){
                for(int i=0;i<recurso_perfil.size();i++){
                    Recursos_Perfil recurso_perfil2 = recurso_perfil.get(i);
                     %>
                        <tr>
                            <td><div align="center"><span class="Estilo4"><%=i+1%></span></div></td>
                            <td><div align="center"><span class="Estilo4"><%= recurso_perfil2.getRPNOMBRE() %></span></div></td>
                         </tr>
        <%
                }
        }
