            ...
            function buscarIn(){
              if(document.form.fechaInR.value.length==10 && document.form.fechaFnR.value.length==10){
                if(document.form.busqueda[0].checked){
                    window.frames[0].location.href="VerReincidentes.jsp?fi="+document.form.fechaInR.value+"&ff="+document.form.fechaFnR.value+"&doc="+document.form.busqueda[0].value+"&excel=1";
                }else{
                    window.frames[0].location.href="VerReincidentes.jsp?fi="+document.form.fechaInR.value+"&ff="+document.form.fechaFnR.value+"&doc="+document.form.busqueda[1].value+"&excel=1";
                }
            }else{
            alert("Digite fechas");
            }
...
//              alert("funcion");
              if(document.form.fechaInR.value.length==10 && document.form.fechaFnR.value.length==10){
                if(document.form.busqueda[0].checked){
                    window.frames[0].location.href="VerReincidentes.jsp?fi="+document.form.fechaInR.value+"&ff="+document.form.fechaFnR.value+"&doc="+document.form.busqueda[0].value+"&excel=0";
                }else{
                    window.frames[0].location.href="VerReincidentes.jsp?fi="+document.form.fechaInR.value+"&ff="+document.form.fechaFnR.value+"&doc="+document.form.busqueda[1].value+"&excel=0";
                }
            }else{
            alert("Digite fechas");
            }
...
-->
    ...
    <body> 
		...
		<form name="form" id="form" method="post" action="javascript:buscar()" onSubmit="">
		    ...
		    <legend class="style5"  align="center">CONSULTAR&nbsp;REINCIDENTES</legend>
		  	</div>
            <table border="0" width="100%" id="tabla5" style="display:" >
                
								<tr valign="middle">
								   <td width="12%" align="right"><label style="color:#333333"><span class="style5">FECHA&nbsp;INICIAL</span></label>
								  </td>
...
<td width="27%" scope="col" class="Estilo16"><input type="radio" name="busqueda" id="busqueda" value="1" checked="checked" onClick="">
				  Docuemnto</span>&nbsp;&nbsp;
                                  <input type="radio" name="busqueda" id="busqueda" value="2"  onClick="">
				  Placa</span></td>
<td width="4%" scope="col"><input id="exel"  name="exel" type="button" value="Excel" style="color:#333333" onclick="buscarIn()" /></td>
				<td width="19%" scope="col" align="center">
                                    <input id="reporte"  name="reporte" type="submit" value="Reporte" style="color:#333333" /></td>
                        <TD width="0%">&nbsp;</TD>
								</tr>
						</table>
