    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
            function consultarPlacas(){
                if(document.form.placa.value.length>=2&&document.form.placa.value.length<=9){
			...
			function liquidacion(id){
				var accept = true;
                                var paso = true;
				var valor = parseFloat(document.getElementById('totalidad').value);
                                var codeSec = "";
                                if(document.getElementById("tipoPagos").value.length>0){
                                    if(parseInt(document.getElementById("tipoPagos").value)!=1){
                                           var valor_tipo = parseFloat(document.getElementById("valor").value==''?0:document.getElementById("valor").value);
					...
					if(document.getElementById("propietario")!=null){
						if(confirm("Esta seguro aplicar esta liquidacion")){
							...
							var persona = document.getElementById("propietario").value;
                                                        var sede = document.getElementById("sede").value;
                                                        var pago = document.getElementById("tipoPagos").value;
                                                        var placa = document.getElementById("placax").value;
          ...
          function HabilitarAutenticar2(){
              //document.form.autenticar.disabled = false;
              if(document.form.liqrunt.checked){
                  document.getElementById('autenticar').disabled = false;
                  document.getElementById('aplicar').disabled = true;
              }else{
                  if(document.form.totalidad.value>0 || document.form.totalidadT.value>0){
                    document.getElementById('aplicar').disabled = false;
                  }
                  document.getElementById('autenticar').disabled = true;
              }

          }

function habilitarReciboRunt() {
                    var tipoLiquidacion = 0;
                    if(document.form.liqrunt.checked == 1){
                        tipoLiquidacion = 1;
                    }
                        var ajax2 = nuevoAjax2();
                        ajax2.open("POST", "habilitarRecibosRunt.jsp", true);
                        ajax2.onreadystatechange = function() {
                            if (ajax2.readyState == 4) {
                        ...
                        ajax2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                        ajax2.send("tipoLiquidacion=" + tipoLiquidacion);
                }
                            ...
                            <tr>
                              <th width="6%" scope="col"><label style="color:#CC0000"><span class="Estilo19">PLACA</span></label></th>
                                <th width="13%" scope="col">
                                    <input id="placa" name="placa" type="text" size="20" maxlength="9" style="text-align:center;color:#333333;text-transform:uppercase" onkeyup="deshBoton()"/>
									...
									<tr>
										...
										<td width="40%" align="right">
											<span class="Estilo19">Generar&nbsp;liquidacion&nbsp;RUNT</span>
                                                                                        <% if(((java.util.LinkedList)session.getAttribute("permisosU")).contains("aplicarPagosRunt")){
                                                                                                if(((java.util.LinkedList)session.getAttribute("permisosU")).contains("aplicarPagosTransito")){ %>
                                                                                                <input type="checkbox" name="liqrunt" id="liqrunt" value="1" onclick="habilitarReciboRunt()" />
                                                                                             <% }else{
                                                                        ...
                                                                        <%
                                                                           ...
                                                                           if(estadoT||aplicarSinTaquilla){  %>
                                                                              <tr>
                                                                                    ...
                                                                                    <th width="20%" scope="col"><input name="autenticar" id="autenticar" type="button" value="Autenticar" onclick="abrirVentanaAutenticar()" style="display:none"/></th>
