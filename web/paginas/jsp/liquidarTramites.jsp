...
function verTramites(placa){
		var clasestramites = document.getElementById("clasestramites");
                var valor1 = 0;
		var valor2 = 0;
		if(clasestramites.value == 1 || clasestramites.value == 4 || clasestramites.value == 5){
			...
			document.getElementById('clasesvehiculo').style.display = 'block';
			...
			valor2 = document.getElementById('servicio').value;
                    ...
                    if(clasestramites.value ==2){
                        ...
                        document.getElementById('clasesvehiculo').style.display = 'none';
		...
		window.frames[0].location.href = "cargarTramites.jsp?clase="+clasestramites.value+"&sede="+sedeT.value+"&valor1="+valor1+"&valor2="+valor2+"&placa="+placa+"&check="+valCheck;
         ...
         function buscarClases(){
            tabla = document.getElementById('clasesvehiculo');
            valor = document.form.clasestramites.options[document.form.clasestramites.selectedIndex].value;
            placa = document.getElementById('placax').value;

            if(valor!=2&&valor!=3){
                ajax1=new nuevoAjax2();
                ajax1.open("POST", "cargarClases.jsp",true);
                ...
                ajax1.send("radicacion="+valor+"&name=clasesvehiculos"+"&placa="+placa);
            }
         }
...
<body>
    <form method="post" id="form" name="form" action="">
        <input type="hidden" name="placax" id="placax" value="<%=request.getParameter("placa") %>">
<% try{
...
%>
                <input type="hidden" name="placax" id="placax" value="<%=request.getParameter("placa").toUpperCase() %>" />
        ...
        <table width="34%" border="0" align="center">
          ...
          <tr>
            <td width="24%">
              ...
              <td width="76%"><select name="clasestramites" id="clasestramites" style="color:#333333; font:bold;font-size:12px; width:250px" onchange="verTramites('<%=placa%>')" onchange="buscarClases(); return false" onblur="buscarClases(); return false" >
							<% try {
							...
							<%                  }
                                                                            ...
                                                                            if(parametro.getCodigo()==2||parametro.getCodigo()==3||parametro.getCodigo()==6){%>
                                                                                <option value="<%=parametro.getCodigo() + ""%>"><%=parametro.getNombre()%></option>
							<%                  }
                                                                        }
                                                                }
          ...
          <tr>
            ...
            <td>
                <div  style="width:150px" align="center" id="clasesvehiculo">
                <select id="clasesvehiculos"  style="color:#333333; font:bold;font-size:12px; width:250px" onchange="verTramites('<%=placa%>')" >
                    <option value="0">---Seleccione Clase---</option>
				<% try {
					java.util.List listaTipos = model.listadoParametros(1);
					for (int i = 0; i < listaTipos.size(); i++) {
						Parametro parametro = (Parametro) listaTipos.get(i);
                                                if(parametro.getNombreCorto().equals("1")){
                                                    if(vehiculo!=null){
