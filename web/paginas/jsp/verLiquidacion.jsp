            ...
            <table width="100%" border="1" cellpadding="1" cellspacing="0">
                ...
                <%
                    try {
                        ...
                        double vrete = 0;
                        if (request.getParameter("valorRetencion") != null) {
                            String valor = request.getParameter("valorRetencion");
                            ...
                            if (!request.getParameter("id").equals("") && !request.getParameter("tipo").equals("")) {
                                ...
                                if (!model.liquidacionExiste(listaLiquidacion, Long.parseLong(request.getParameter("id")), Integer.parseInt(request.getParameter("tipo")))) {
                                    if (request.getParameter("tipo").equals("1")) {
                                        ...
                                        model.actualizarValoresCartera(Long.parseLong(request.getParameter("id")));
                                        model.getCon().commit();
                ...
                %><script>alert("El vehiculo de placa " + '<%=request.getParameter("placa")%>' + " ya se encuentra matriculado ");</script> <%
                    ...
                    if (request.getParameter("id").equals("2") || request.getParameter("id").equals("9")
                            || request.getParameter("id").equals("63") || request.getParameter("id").equals("70")
                            || request.getParameter("id").equals("92") || request.getParameter("id").equals("99")) {
                        String numplaca = request.getParameter("placa");
                        Vehiculo veh = model.consultarVehiculo(numplaca);
                    ...
                    if (request.getParameter("tipo").equals("3")) {
                        if (request.getParameter("id").equals("204")) {
                            if (request.getParameter("placa") != null) {
                                String numplaca = request.getParameter("placa");
                                Vehiculo vehiculo = model.consultarVehiculo(numplaca);
                                if (vehiculo != null) {
                                    if (vrete == 0) {
                %>
                <script language="javascript" type="text/javascript">
                    RetencionF('<%=vehiculo.getId_vehiculo()%>', '<%=numplaca%>');
                </script>
                <%
                                                    } else {
                            ...
                            for (int i = 0; i < listaLiquidacion.size(); i++) {
                                ...
                                if (liquidacion.getTipo() == 1 || liquidacion.getTipo() == 3) {
                                    ...
                                    if (liquidacion.getTipo() == 1) {
                                        model.getCon().setAutoCommit(false);
                                        model.actualizarValoresCartera(liquidacion.getId_liquidacion());
                                        model.getCon().commit();
                                        car = model.getCartera(liquidacion.getId_liquidacion());
                                    }
                                    liquidacion.setTotal(liquidacion.getTipo() == 1 ? (float) car.getSaldo() : liquidacion.getTotal());
                ...
                <tr style="border:none" onMouseOver="sombrear(this)" onMouseOut="renovar(this)">
                    ...
                    <%  Sede sd = (Sede) session.getAttribute("sede");
                        if (model.consultarAtributo(sd.getId_sede(), 1, liquidacion.getFk_concepto()) && (liquidacion.getTipo() == 1 || liquidacion.getTipo() == 1)) {
                    %>
