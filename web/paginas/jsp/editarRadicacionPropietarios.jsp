...
<script language="javascript" type="text/javascript">
	...
	function aplicarP(tipo,i){
		...
		document.location.href = "editarRadicacionPropietarios.jsp?tipo=5&tipopropiedad="+document.getElementById('tipopropiedad'+i).value+"&id="+i;
	}
	...
	function aplicarI(tipo,i){
		...
		document.location.href = "editarRadicacionPropietarios.jsp?tipo=6&proindiviso="+document.getElementById('indivisos'+i).value+"&id="+i;
	}
	function eliminar(i){
		document.location.href = "editarRadicacionPropietarios.jsp?tipo=4&id="+i;
	}
		...
		ajax2.onreadystatechange=function() {
			if (ajax2.readyState==4) {
				...
				if(parseInt(respuesta) == 0){
					...
					document.location.href = "editarRadicacionPropietarios.jsp?tipo=2&tipod="+dato3+"&documento="+dato4+"&id="+id;
				}else{
	...
	function validarP(campoP,id){
		...
		document.location.href = "editarRadicacionPropietarios.jsp?tipo=3&porcentaje="+campoP.value+"&id="+id;
	}

	function addApoderado(prop){
		if(prop>0)
		document.location.href = "editarRadicacionApoderados.jsp?tipo=1&propietario="+prop;
	}
  ...
  <% java.util.List lista =(java.util.List)session.getAttribute("editarPropietarios");
	 ...
	 if(request.getParameter("tipo")!=null){
		 if(request.getParameter("tipo").equals("1")){
			...
			session.setAttribute("editarPropietarios",lista);
		 }else{
...
<%	}
	...
	for(int i=0;i<lista.size();i++){
	  ...
	  <tr>
		<td align="center"><input name="ttipo<%=i%>" type="text" id="ttipo<%=i%>" ondblclick="verT(this,<%=i%>)" value="<%=tipos2.get((persona!=null?persona.getTipo_documento():1)+"")%>" readonly="true" style="text-align:center; color:#333333; font-family:Arial, Helvetica, sans-serif; width:130px; font-size:11px"/>
		  <select name="tipo<%=i%>" id="tipo<%=i%>" style="display:none; font-size:11px; width:130px" onchange="aplicarT(this,<%=i%>)" >
		...
		<td align="center"><input name="tdocumento<%=i%>" type="text" id="tdocumento<%=i%>" value="<%=(persona!=null?persona.getDocumento():"")%>" onblur="consultaPersona(this,<%=i%>)" style="text-align:center; color:#333333; font-family:Arial, Helvetica, sans-serif; width:100px; font-size:11px"/></td>
		<td align="center">
			<%  if(persona!=null){%>
				<input name="tnombre<%=i%>" type="text" id="tnombre<%=i%>" value="<%=persona.getApellido_1() + " " + (persona.getApellido_2() != null ? persona.getApellido_2() : "") + " " + persona.getNombre_1()%>" readonly="true" style="color:#333333; font-family:Arial, Helvetica, sans-serif; width:250px; font-size:11px" />
                            <%}else{ %>
				<input name="tnombre<%=i%>" type="text" id="tnombre<%=i%>" value="" readonly="true" style="color:#333333;font-family:Arial, Helvetica, sans-serif; width:250px; font-size:11px" />
            <%		}
	...
	<td align="center">
         ...
         <input name="ttipopropiedad<%=i%>" type="text" id="ttipopropiedad<%=i%>" ondblclick="verP(this,<%=i%>)" value="<%=tipospropiedades2.get((propietario.getTipoPropiedad()!=null?propietario.getTipoPropiedad():"1")+"")%>" readonly="true" style="text-align:center; color:#333333; font-family:Arial, Helvetica, sans-serif; width:130px; font-size:11px"/>
			<select name="tipopropiedad<%=i%>" id="tipopropiedad<%=i%>" style="display:none; font-size:11px; width:130px" onchange="aplicarP(this,<%=i%>)">
			...
			<td align="center">
                            ...
                            <input name="tindivisos<%=i%>" type="text" id="tindivisos<%=i%>" ondblclick="verI(this,<%=i%>)" value="<%=propietario.getProIndiviso()!=null?propietario.getProIndiviso():"N"%>" readonly="true" style="text-align:center; color:#333333; font-family:Arial;width:30px; font-size:11px"/>
			<select name="indivisos<%=i%>" id="indivisos<%=i%>" style="display:none; width:30px; font-size:11px" onchange="aplicarI(this,<%=i%>)">
		...
		<td align="center"><input name="tporcentaje<%=i%>" type="text" id="tporcentaje<%=i%>" value="<%=(int)propietario.getPorcentaje()%>" onblur="validarP(this,<%=i%>)" style="text-align:center; color:#333333; font-family:Arial, Helvetica, sans-serif; width:30px; font-size:11px"/></td>
		<td align="center">
		<input type="button" name="add<%=i%>" id="add<%=i%>" value="+" style="font-size:18px; height:20px; text-align:center; cursor:pointer; width:15px; vertical-align:middle; margin-left:0px; margin-top:0px" onclick="addApoderado(<%=propietario.getFk_persona()%>)"/>
</td>
		<td align="center">
