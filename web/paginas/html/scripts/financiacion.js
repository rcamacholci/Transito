...
function consultarPlacas(){
    ...
    if(placa.value.length>=5 && placa.value.length<=6){
        window.frames.iframeCartera.location.href = 'verCarteraFinanciacion.jsp?id=1&placa='+placa.value;
...
function consultarPersonas(){
    ...
    if(documento.value.length>=5 ){//&& placa.value.length<=11){
        ...
        window.frames.iframeCartera.location.href = 'verCarteraFinanciacion.jsp?id=2&documento='+documento.value+'&tipo='+tipo.value;
        window.frames.iframeTipo.location.href = 'calcularTipoFinanciacion.jsp';
        Documento_Consultado = documento.value
...
function actualizarTipoF(check){
    if(check.checked){
        window.parent.frames[1].location.href = "calcularTipoFinanciacion.jsp?id=1&add="+check.id;
    }else{
        window.parent.frames[1].location.href = "calcularTipoFinanciacion.jsp?id=0&add="+check.id;
    }
}
