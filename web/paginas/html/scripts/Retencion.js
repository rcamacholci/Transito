...
function MetodoRetencion(numPlaca, optn) {
    var placa;
    var avaluo;
    if(optn == '1'){
        placa = numPlaca.value;
    }else{
        placa = document.getElementById('placa').value;
        avaluo = numPlaca.value;
    }
    
    ajax = nuevoAjax();
    ajax.open("POST", "ProcesarEditarRetencion.jsp");
    ajax.onreadystatechange = function () {
        if (ajax.readyState == 4) {
            var docxml = ajax.responseXML;
            var stats = docxml.getElementsByTagName('stats')[0].childNodes[0].nodeValue;
            if (stats == '0') {
                var valorRetencion = docxml.getElementsByTagName('valorRetencion')[0].childNodes[0].nodeValue;
                alert(valorRetencion)
                document.getElementById('Avaluo').value = valorRetencion;
            } else {
                if (stats == '1') {
                    alert("exito");
                    ...
                    var Mnsj = docxml.getElementsByTagName('Mnsj')[0].childNodes[0].nodeValue;
                    alert(Mnsj);
                }
            }
        }
    ...
    ajax.send("placa=" + placa + "&optn=" + optn + "&avaluo="+avaluo);
}
